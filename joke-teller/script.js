...
const test = () => {
  VoiceRSS.speech({
    key: 'b2f5bce25dfc4e37950dfa8ad7537e33',
    src: 'Hello, world!',
    hl: 'en-us',
    v: 'Linda',
    r: 0, 
...
const getJokes = async () => {
  ...
  try {
    ...
    console.log('error in getJokes function: ', error)
  }
}

getJokes();

const API_KEY = 'b2f5bce25dfc4e37950dfa8ad7537e33';
let message = 'hello world!'


const getVoiceData = async () => {
  try {
    const response = await fetch(`https://api.voicerss.org/?key=${API_KEY}&hl=en-us&src=${message}`);
    const data = await response.json();
    console.log(data)
  } catch (error) {
